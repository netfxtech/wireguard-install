...
# Slight modifications made to Angristan's Secure WireGuard server installer
# https://github.com/netfxtech/wireguard-install
...
function installQuestions() {
	...
	echo "The git repository is available at: https://github.com/netfxtech/wireguard-install"
	echo ""
...
function newClient() {
	...
	# Where the client configuration will be written
	if [ -e "/etc/cli" ]; then
		HOME_DIR="/etc/wireguard/clients/${CLIENT_NAME}"
	else 
...
AllowedIPs = ${CLIENT_WG_IPV4}/32,${CLIENT_WG_IPV6}/128" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	echo "The config and PNG have also been saved at the following locations:"
	echo "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
	echo "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.png"
}
...
function revokeClient() {
	...
	rm -f "/etc/wireguard/clients/${SERVER_WG_NIC}-client-${CLIENT_NAME}.png"
